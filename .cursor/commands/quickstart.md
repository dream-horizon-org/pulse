...
3. Build services: `cd deploy && ./scripts/build.sh` or `./scripts/build.sh all` (default: ui + server + cron + session capture + session replay + heatmap screenshot ingestion + pulse-ai-agent)
