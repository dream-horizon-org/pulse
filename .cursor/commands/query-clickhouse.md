...
Useful reference tables: `otel.otel_traces`, `otel.otel_logs`, `otel.otel_metrics` (view over all OTLP metric types), physical metric tables `otel.otel_metrics_gauge`, `otel.otel_metrics_sum`, `otel.otel_metrics_summary`, `otel.otel_metrics_histogram`, `otel.otel_metrics_exp_histogram`, `otel.stack_trace_events`, `otel.interaction_heatmaps_daily` (heatmap DDL: `backend/ingestion/clickhouse-otel-schema.sql`)
