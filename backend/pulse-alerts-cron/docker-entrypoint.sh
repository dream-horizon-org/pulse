...
if [ "${PULSE_BACKEND_OTEL_ENABLED}" = "true" ] && [ -r /otel/opentelemetry-javaagent.jar ]; then
  ...
  export OTEL_SERVICE_NAME="${OTEL_SERVICE_NAME:-pulse-alerts-cron}"
else
...
exec java ${JAVA_AGENT} -jar pulse-alerts-cron.jar run org.dreamhorizon.pulsealertscron.verticle.MainVerticle
