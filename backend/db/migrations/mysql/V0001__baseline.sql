...
--changeset db-migrations:V0001__baseline runOnChange:false failOnError:true splitStatements:true endDelimiter:;
--comment Full baseline: 38 tables in pulse_db (as of Liquibase adoption).
--comment EXISTING ENVIRONMENTS: run changelogSync via the standalone Jenkins pipeline before first deployment.
--comment NEW ENVIRONMENTS: this executes on first update and builds the full schema from scratch.
