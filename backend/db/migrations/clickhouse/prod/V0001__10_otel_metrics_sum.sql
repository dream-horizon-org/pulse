...
--changeset db-migrations:V0001__10_otel_metrics_sum runOnChange:false failOnError:true splitStatements:true endDelimiter:; dbms:clickhouse
--comment Baseline (prod): V0001__10_otel_metrics_sum.sql

CREATE TABLE IF NOT EXISTS otel.otel_metrics_sum_local
  ON CLUSTER 'pulse-ch'
(
  ...
  ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/otel/otel_metrics_sum_local','{replica}')
...
AS otel.otel_metrics_sum_local
  ENGINE = Distributed('pulse-ch', otel, otel_metrics_sum_local, cityHash64((ProjectId, MetricName)));
