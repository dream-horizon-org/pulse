...
--changeset db-migrations:V0001__12_otel_metrics_exp_histogram runOnChange:false failOnError:true splitStatements:true endDelimiter:; dbms:clickhouse
--comment Baseline (prod): V0001__12_otel_metrics_exp_histogram.sql

CREATE TABLE IF NOT EXISTS otel.otel_metrics_exp_histogram_local
  ON CLUSTER 'pulse-ch'
(
  ...
  `Scale`                        Int32                               CODEC(T64, ZSTD(1)),
  `ZeroCount`                    UInt64                              CODEC(T64, ZSTD(1)),
  `PositiveOffset`               Int32                               CODEC(T64, ZSTD(1)),
  `PositiveBucketCounts`         Array(UInt64)                       CODEC(T64, ZSTD(1)),
  `NegativeOffset`               Int32                               CODEC(T64, ZSTD(1)),
  `NegativeBucketCounts`         Array(UInt64)                       CODEC(T64, ZSTD(1)),
  ...
  ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/otel/otel_metrics_exp_histogram_local', '{replica}')
...
CREATE TABLE IF NOT EXISTS otel.otel_metrics_exp_histogram
  ON CLUSTER 'pulse-ch'
AS otel.otel_metrics_exp_histogram_local
  ENGINE = Distributed('pulse-ch', otel, otel_metrics_exp_histogram_local, cityHash64((ProjectId, MetricName)));
