...
public class AthenaModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    bind(AthenaService.class).in(Singleton.class);
  }
  ...
  private static class AthenaAsyncClientProvider implements Provider<AthenaAsyncClient> {
    ...
    public AthenaAsyncClient get() {
      ...
      if (region == null || region.isEmpty()) {
        throw new IllegalStateException("Athena region must be configured via CONFIG_SERVICE_APPLICATION_ATHENA_REGION environment variable or config file");
      }
