package org.dreamhorizon.pulseserver.dao.rcareport;

import com.google.inject.Inject;
import com.google.inject.Singleton;
import io.reactivex.rxjava3.core.Completable;
import io.reactivex.rxjava3.core.Maybe;
import io.vertx.rxjava3.sqlclient.Row;
import io.vertx.rxjava3.sqlclient.Tuple;
import java.time.Instant;
import java.time.LocalDate;
import java.time.LocalDateTime;
import java.time.ZoneOffset;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.dreamhorizon.pulseserver.client.mysql.MysqlClient;
import org.dreamhorizon.pulseserver.dao.rcareport.models.RcaReportCacheHit;

@Slf4j
@Singleton
@RequiredArgsConstructor(onConstructor = @__({@Inject}))
public class ScreenRcaNarrativeCacheDao {

  private final MysqlClient mysqlClient;

  /** Returns cached narrative JSON if present for the key (no TTL). */
  public Maybe<RcaReportCacheHit> get(
      String projectId,
      String screenName,
      LocalDate windowStartDateUtc,
      LocalDate windowEndDateUtc,
      String payloadFingerprintHex) {
    return mysqlClient
        .getReaderPool()
        .preparedQuery(ScreenRcaNarrativeCacheQueries.GET_BY_KEY)
        .rxExecute(
            Tuple.of(projectId, screenName, windowStartDateUtc, windowEndDateUtc, payloadFingerprintHex))
        .flatMapMaybe(
            rows -> {
              if (rows.size() == 0) {
                return Maybe.empty();
              }
              Row row = rows.iterator().next();
              String body = row.getString(0);
              if (body == null || body.isBlank()) {
                return Maybe.empty();
              }
              LocalDateTime cachedAtLdt = row.getLocalDateTime(1);
              Instant cachedAt =
                  cachedAtLdt != null ? cachedAtLdt.toInstant(ZoneOffset.UTC) : null;
              return Maybe.just(new RcaReportCacheHit(body, cachedAt));
            })
        .doOnError(
            e ->
                log.warn(
                    "Screen RCA narrative cache get failed: {}", e.getMessage()));
  }

  public Completable put(
      String projectId,
      String screenName,
      LocalDate windowStartDateUtc,
      LocalDate windowEndDateUtc,
      String payloadFingerprintHex,
      String reportBody) {
    if (reportBody == null || reportBody.isBlank()) {
      return Completable.complete();
    }
    return mysqlClient
        .getWriterPool()
        .preparedQuery(ScreenRcaNarrativeCacheQueries.UPSERT)
        .rxExecute(
            Tuple.of(
                projectId,
                screenName,
                windowStartDateUtc,
                windowEndDateUtc,
                payloadFingerprintHex,
                reportBody))
        .ignoreElement()
        .doOnComplete(
            () ->
                log.debug(
                    "Screen RCA narrative cached: project={}, screen={}, startDate={}, endDate={}",
                    projectId,
                    screenName,
                    windowStartDateUtc,
                    windowEndDateUtc))
        .doOnError(
            e ->
                log.warn(
                    "Screen RCA narrative cache put failed: {}", e.getMessage()));
  }
}
