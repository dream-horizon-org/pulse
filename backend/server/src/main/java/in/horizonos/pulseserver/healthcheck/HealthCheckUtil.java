package in.horizonos.pulseserver.healthcheck;

import in.horizonos.pulseserver.rest.exception.HealthCheckException;
import in.horizonos.pulseserver.util.MapUtils;
...
public class HealthCheckUtil {
  ...
  public static Single<HealthCheckResponse> handler(Map<String, Single<JsonObject>> healthChecks) {
    ...
    Single<HealthCheckResponse> finalHealthChecks = Single.zip(
        healthChecksWithType,
        checks -> new HealthCheckResponse(Arrays.asList(Arrays.copyOf(checks, checks.length, HealthCheckResponse.Check[].class))));

    return finalHealthChecks.map(response -> {
      if (response.getStatus().equals(HealthCheckResponse.Status.DOWN)) {
        // Given the limitation of out current d11-rest implementation, a hacky way to make it throw 500 error with given body
