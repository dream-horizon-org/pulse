...
mod tests {
    ...
    async fn test_extract_body_no_timeout() {
        ...
        let result =
            extract_body_with_timeout(body, 1024, None, TEST_CHUNK_SIZE_KB, "/test").await;
        assert!(result.is_ok());
    ...
    async fn test_extract_body_exceeds_limit() {
        ...
        let result =
            extract_body_with_timeout(body, 10, None, TEST_CHUNK_SIZE_KB, "/test").await;
        assert!(matches!(result, Err(CaptureError::PayloadTooBig(_))));
    }
