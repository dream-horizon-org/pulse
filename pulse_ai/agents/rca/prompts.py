...
1. **Trust the server list:** Pulse already applies a **baseline raw-rate gate** before segments reach you: interaction RCA keeps cohorts whose **`metrics.error_rate` + `metrics.poor_user_pct`** is **strictly greater** than **`baseline.error_rate` + `baseline.poor_user_pct`** (same 0–100 scale as payload). Rows in **`segments[]`** survived that gate. Screen RCA requires **`bad_frustration` strictly greater than baseline** on the analogous payload. **Do not drop a segment because of a “10% of historical volume” rule** — per-segment **historical** volume baselines are **not** present in this payload.
...
4. **Optional noise trim (only when both degrading):** If both rates degrade but the lifts are **tiny** on the **0–100** scale, you may treat as non-actionable: **absolute** interaction-level lifts **(segment − baseline)** below **2.0** percentage points on `error_rate` **and** below **5.0** percentage points on `poor_user_pct`. Prefer this only when the raw-rate margin over baseline is modest; do **not** discard cohorts the server kept solely because this optional band disagrees.
