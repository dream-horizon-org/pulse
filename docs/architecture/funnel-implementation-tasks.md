...
| ID  | Task                                                                                                                                                                                                                                             | Notes                                                                     |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------- |
| B5  | Add `POST /v1/funnel` (save): validate body (name, steps, windowSeconds, mode, dateRangeDays), persist to `funnel`, create `funnel_job` row (PENDING), trigger Spark job (Glue/EMR) with mode=on_save, funnel_id, project_id, date_from, date_to | Return 202 Accepted with funnelId and jobId. Project/tenant from context. |
...
| S5  | Apply per-funnel filters (from steps_json/filters_json) as Spark filter expressions                                                                                                                                             | build_filter_expr(funnel).               |
...
| ID  | Task                                                                                                                                                                       | Notes                                     |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| U9  | Keep existing ad-hoc “Analyze” flow (POST /v1/funnel/analyze) for exploration; add entry points to “Save” from current builder and to “Saved funnels” list                 | No breaking change to current analyze UX. |
