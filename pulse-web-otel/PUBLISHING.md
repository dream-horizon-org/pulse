...
> Audience: maintainers shipping the Pulse Web SDK to npm for the first time and on every subsequent release. This is the single canonical source for packaging, versioning, release flow, and the long-running "subpath vs sibling package" decision.

Sibling docs:

- `pulse-web-otel/README.md` — consumer install + quick wiring
- `pulse-web-otel/web-sdk-plan/INTEGRATION.md` — full consumer integration guide (React / Next.js / vanilla)
- `pulse-web-otel/CHANGELOG.md` — release notes (currently `0.1.0-alpha.x`)
- `.github/workflows/publish-react-native.yml` — sister-SDK publish workflow we should mirror for the web SDK
...
- `web-sdk-plan/`, `examples/`, `node_modules/`, `src/`, `tsup.config.ts`, `vitest.config.ts`, `.size-limit.json` are **excluded** because they are not under `dist/`. Good.
...
Full integration recipes live in `pulse-web-otel/web-sdk-plan/INTEGRATION.md`; do not duplicate them here.
...
4. **Single `INTEGRATION.md`.** Splitting docs across packages is friction we don't need pre-1.0.
...
The signal contract is the **public API**. A change in `pulse.type`, a span attribute name, or a log body schema is a major. Reference: `pulse-web-otel/src/instrumentations/`* plus the contract docs under `pulse-web-otel/web-sdk-plan/v*-*/04-contract-parity.md`.
...
- Sentry monorepo overview — [https://mintlify.com/getsentry/sentry-javascript/contributing/architecture/monorepo](https://mintlify.com/getsentry/sentry-javascript/contributing/architecture/monorepo)
