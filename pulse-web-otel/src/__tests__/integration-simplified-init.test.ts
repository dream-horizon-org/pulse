...
class MockClickHouseExporter {
  ...
  exportSpan(span: any) {
    ...
    this.spans.push({ ...span, exportedAt: Date.now(), destination: this.baseUrl });
  }

  exportLog(log: any) {
    console.log(`[Mock CH] Exporting log to ${this.baseUrl}: ${log.body}`);
    this.logs.push({ ...log, exportedAt: Date.now(), destination: this.baseUrl });
  }
...
describe('Simplified SDK Initialization Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    prodExporter = new MockClickHouseExporter('https://collector.prod.example.com');
  });
  ...
  describe('Production Mode (non-devkey)', () => {
    ...
    it('should export spans to production collector', () => {
      ...
      expect(exported).toHaveLength(1);
      expect(exported[0].name).toBe('http.request');
      expect(exported[0].destination).toBe('https://collector.prod.example.com');
    });
  ...
  describe('Data Flow Verification', () => {
    ...
    it('should flow multiple signals through prod mode without data loss', () => {
      ...
      const baseUrl = resolveEndpointBaseUrl(prodKey, 'https://collector.prod.example.com');
