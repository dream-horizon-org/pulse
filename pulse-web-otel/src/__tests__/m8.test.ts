...
describe("TC 8.1 — pagehide registered once on start()", () => {
  it("adds a single pagehide listener to window", async () => {
    ...
    // sdk.ts adds 1; session.ts adds 1; navigation instrumentation may add 1 more.
    // Exact count varies by active instrumentations — assert at least 1, at most one per registrant.
