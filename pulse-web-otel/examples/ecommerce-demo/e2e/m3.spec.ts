...
test.describe("@M3 errors", () => {
  test("uncaught window.onerror → device.crash log with stack trace", async ({
    page,
    otlp,
  }) => {
  ...
  test("unhandled Promise rejection → non_fatal log", async ({
    page,
    otlp,
  }) => {
    await page.goto("/error-demo");
  ...
  test("same error repeated → only 1 log (deduplication)", async ({
    page,
    otlp,
  }) => {
    await page.goto("/error-demo");
  ...
  test('cross-origin "Script error." → skipped (not emitted)', async ({
    page,
    otlp,
  }) => {
    await page.goto("/");
...
test.describe("@M3 network", () => {
  test("fetch() → http span with method, url, status_code", async ({
    page,
    otlp,
  }) => {
  ...
  test("GraphQL POST → http span with graphql.operation.name", async ({
    page,
    otlp,
  }) => {
    await page.goto("/");
    ...
    const span = await otlp.waitForSpan("http");
    expect(getAttr(span.attributes, "graphql.operation.name")).toBe(
      "GetProducts",
    );
  });
});
...
test.describe("@M3 clicks", () => {
  test("click on ProductCard → app.click log with coordinates", async ({
    page,
    otlp,
  }) => {
    await page.goto("/products");
  ...
  test("3 rapid clicks on RageClickButton → rage_click: true", async ({
    page,
    otlp,
  }) => {
    await page.goto("/products");
    ...
    const rageLogs = logs.filter(
      (l) => getAttr(l.attributes, "rage_click") === true,
    );
...
test.describe("@M3 web vitals", () => {
  test("LCP metric emitted after page load", async ({ page, otlp }) => {
    ...
    expect(getAttr(dp.attributes, "metric.rating")).toMatch(
  ...
  test("metric data points include global attributes (session.id, installation.id, screen.name, platform)", async ({
    page,
    otlp,
  }) => {
    await page.goto("/products");
    ...
    await page.evaluate(async () => {
      ...
      if (!sdk) {
        console.error("[test] PulseWeb not found on window");
        return;
      }
      ...
      if (!mp) {
        console.error("[test] meterProvider not found on PulseWeb");
        return;
      }
  ...
  test.skip("INP metric emitted after user interaction", async ({
    page,
    otlp,
  }) => {
...
test.describe("@M3 navigation", () => {
  test("initial page load → screen_load span with ttfb_ms", async ({
    page,
    otlp,
  }) => {
    await page.goto("/products");
  ...
  test("screen_load span includes load.duration_ms", async ({ page, otlp }) => {
    ...
    const span = await otlp.waitForSpan("screen_load");
    expect(
      Number(getAttr(span.attributes, "load.duration_ms") ?? -1),
    ).toBeGreaterThan(0);
  });

  test("SPA route change → screen_session span with screen.name", async ({
    page,
    otlp,
  }) => {
    await page.goto("/");
...
test.describe("@M3 consent gate across all instrumentations", () => {
  test("DENIED consent → zero signals from all 5 instrumentations", async ({
    page,
    otlp,
  }) => {
    ...
    await page.evaluate(async () => {
      ...
      window.dispatchEvent(
        new ErrorEvent("error", { message: "test", error: new Error("test") }),
      );
      // Network
...
test.describe("@M3 graceful no-ops on non-Chrome browsers", () => {
  test("no uncaught errors in Firefox (longtask PerformanceObserver unavailable)", async ({
    page,
  }) => {
    const errors: string[] = [];
