...
test.describe('@M2 interaction tracking', () => {
  test('checkout flow emits interaction span with APDEX user_category', async ({ page, otlp }) => {
  ...
  test('interaction timeout mid-sequence → no span emitted, no crash', async ({ page, otlp }) => {
    await page.goto('/checkout');
  ...
  test('rapid checkout (< apdex_t) → user_category Satisfied', async ({ page, otlp }) => {
    await page.goto('/checkout');
...
test.describe('@M2 SDK config + sampling', () => {
  test('config fetch failure → SDK still works, no crash', async ({ page, otlp }) => {
  ...
  test('sessionSampleRate: 0 in remote config → zero signals exported', async ({ page, otlp }) => {
    // Serve remote SDK config with 0 sampling
    await page.route('**/v1/configs/active', route =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ sessionSampleRate: 0, features: {} }),
      }),
    );
    // Give config time to load, then clear any pre-config signals
    await page.goto('/');
    await page.waitForTimeout(500);
    otlp.reset();

    // Trigger user actions that would normally emit signals
  ...
  test('feature disabled in remote config → instrumentation not installed', async ({ page, otlp }) => {
    await page.route('**/v1/configs/active', route =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ features: { errors: false, clicks: false } }),
      }),
    );
    await page.goto('/');
    await otlp.waitForLog('session.start'); // SDK still inits
    otlp.reset();

    // Throw an error — errors instrumentation should be off
    await page.evaluate(() => { throw new Error('should not be tracked'); });
    await page.waitForTimeout(500);
...
test.describe('@M2 React integration', () => {
  ...
  test('screen.name heuristic: /products/123 → "products/:id"', async ({ page, otlp }) => {
    await page.goto('/');
    otlp.reset();
  ...
  test('PulseErrorBoundary catches React render error → device.crash log', async ({ page, otlp }) => {
  ...
  test('SSR guard: no localStorage-is-not-defined error in console', async ({ page }) => {
    const errors: string[] = [];
    page.on('console', msg => { if (msg.type() === 'error') errors.push(msg.text()); });
...
test.describe('@M2 consent gate', () => {
  test('PulseDataCollectionConsent.DENIED → zero signals from all instrumentations', async ({ page, otlp }) => {
