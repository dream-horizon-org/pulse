...
const VALUE: React.CSSProperties = { color: '#a5f3fc', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' };
...
async function countIdbSignals(): Promise<number> {
  ...
  return new Promise(resolve => {
    const req = indexedDB.open('pulse_signal_buffer', 1);
    req.onerror = () => resolve(0);
    req.onsuccess = () => {
      const db = req.result;
      if (!db.objectStoreNames.contains('signals')) { resolve(0); return; }
...
function readStorage() {
  ...
  let configVersion = '—';
  if (raw) {
    try { configVersion = `v${(JSON.parse(raw) as { version: number }).version}`; }
    catch { configVersion = 'invalid JSON'; }
  }
...
export function PulseDebugPanel() {
  ...
  useEffect(() => {
    ...
    window.fetch = async (...args: Parameters<typeof window.fetch>) => {
      ...
      if (match) {
        ...
        setCalls(prev => [
          { endpoint, time: new Date().toLocaleTimeString(), status: res.status, sizeBytes: buf.byteLength },
          ...prev.slice(0, 29),
        ]);
      }
      return res;
    };
    return () => { if (origFetch.current) window.fetch = origFetch.current; };
  }, []);

  const refresh = useCallback(async () => {
    setStorage(readStorage());
    setIdbCount(await countIdbSignals());
    const pw = (window as Record<string, unknown>)['PulseWeb'] as PulseWebInstance | undefined;
  ...
  if (!open) {
    return (
      <div style={BADGE_STYLE} onClick={() => setOpen(true)} title="Pulse Debug (Shift+P)">
        P
      </div>
    );
  }
  ...
  return (
    <div style={PANEL_STYLE}>
      ...
      <div style={{ marginBottom: 10 }}>
        <div style={{ color: '#64748b', fontSize: 10, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 1 }}>SDK State</div>
        <div style={ROW}>
          <span style={LABEL}>initialized</span>
          <span style={{ color: sdkReady ? '#34d399' : '#f87171' }}>{sdkReady ? '✓ yes' : '✗ no'}</span>
        </div>
      </div>

      {/* LocalStorage */}
      <div style={{ marginBottom: 10 }}>
        <div style={{ color: '#64748b', fontSize: 10, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 1 }}>LocalStorage</div>
        <div style={ROW}>
          <span style={LABEL}>pulse_installation_id</span>
          <span style={VALUE} title={storage.iid}>{short(storage.iid)}</span>
        </div>
        <div style={ROW}>
          <span style={LABEL}>pulse_sdk_config</span>
          <span style={{ color: storage.configVersion === '—' ? '#94a3b8' : '#a5f3fc' }}>{storage.configVersion}</span>
        </div>
        <div style={ROW}>
          <span style={LABEL}>IDB signal buffer</span>
          <span style={{ color: idbCount > 0 ? '#fbbf24' : '#94a3b8' }}>{idbCount} pending</span>
        </div>
      </div>

      {/* OTLP Traffic */}
      <div>
        <div style={{ color: '#64748b', fontSize: 10, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 1 }}>
          OTLP Calls ({calls.length})
          {calls.length > 0 && (
            <button
              onClick={() => setCalls([])}
              style={{ background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontSize: 9, marginLeft: 8 }}
            >
              clear
            </button>
          )}
        ...
        {calls.length === 0 ? (
          <div style={{ color: '#475569', paddingTop: 4 }}>No calls yet — waiting for batch flush…</div>
        ) : (
          calls.map((c, i) => (
            <div key={i} style={{ ...ROW, alignItems: 'center' }}>
              <span style={{ color: '#64748b', minWidth: 62 }}>{c.time}</span>
              {tag(c.endpoint)}
              <span style={{ color: c.status === 200 ? '#34d399' : '#f87171', marginLeft: 4 }}>{c.status}</span>
              <span style={{ color: '#475569', marginLeft: 'auto' }}>{(c.sizeBytes / 1024).toFixed(1)}KB</span>
            </div>
          ))
        )}
      </div>
      ...
      {/* Quick actions */}
      <div style={{ marginTop: 12, paddingTop: 8, borderTop: '1px solid #1e293b', display: 'flex', gap: 8 }}>
        <button
          onClick={() => {
            const p = (window as Record<string, unknown>)['PulseWeb'] as { trackEvent?: (n: string) => void } | undefined;
        ...
        <span style={{ color: '#334155', fontSize: 9, marginLeft: 'auto', alignSelf: 'center' }}>Shift+P to toggle</span>
      </div>
    </div>
  );
}
