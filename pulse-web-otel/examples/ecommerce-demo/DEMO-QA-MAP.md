...
3. `Pulse` is exposed on `window` for manual/E2E (`_PulseExpose`).
4. `_PulseRouterTracking` calls `useRouterTracking({ skipInitial: false })` so **`screen.name` updates on navigation** (pathname). This stamps subsequent signals; it does **not** by itself emit a “navigation” log unless Navigation instrumentation is installed and designed to do so.

## Routes (verify in `src/App.tsx`)

| Route | Screen |
|-------|--------|
| `/` | Home — “Shop Now” uses `Pulse.trackEvent('shop_now_click')`; **Web Vitals QA** panel (`WebVitalsManualTriggers`) — CLS toggle box, INP slow-handler button, TTFB reload note |
| `/products` | Products |
...
| Click “Shop Now” on Home | `Pulse.trackEvent('shop_now_click')` | With **`custom_events`** gate: **custom_event** log. With data collection + **interaction** gate: interaction pipeline. Both can apply — check active mock config for `custom_events` / `interaction` |
