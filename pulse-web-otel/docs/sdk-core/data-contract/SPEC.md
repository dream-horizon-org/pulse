...
| `web_vital`                    | Log    | `WebVitalsInstrumentation`                                             | LCP, CLS, INP, FCP, TTFB (`web-vitals` ^5.x; FID removed upstream)                                                                                                                                        |
...
| `custom_span`                  | Span   | `Pulse.startSpan` / `Pulse.trackSpan`                                  | Host-app manual spans with rich API (`addEvent`, `setAttributes`, `recordException`); see [`../../instrumentations/custom-span/SPEC.md`](../../instrumentations/custom-span/SPEC.md)                      |
...
| `navigation_id`              | `string` | `NavigationInstrumentation` → `setNavigationId()` | No    | UUID v4 per navigation (cold bootstrap in `emitInitialLoadSignals`, fresh UUID before SPA/BFCache `screen_load`); **omitted** from attrs when not yet set — join vitals to `screen_load` |
...
| `web_vital` | `src/__tests__/web-vitals-instrumentation.test.ts`, `global-attrs-processor.test.ts` (`navigation_id`); `integration-simplified-init.test.ts` / `sdk-lifecycle.test.ts` (consent blocks init → no vitals) | `@WebVitals` · `web-vitals.spec.ts`; nextjs-demo `e2e/web-vitals.spec.ts` | In gate (ecommerce); Next optional local run |
