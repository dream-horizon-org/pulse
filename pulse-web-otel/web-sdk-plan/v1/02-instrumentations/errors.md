...
export class ErrorInstrumentation implements PulseInstrumentation {
  ...
  install(_sdk: SdkContext): void {
    ...
    this.onErrorHandler = (e: ErrorEvent) => {
      ...
      logger.emit({
        body: error.message,
        timestamp: Date.now(),           // exact moment error fired
        ...
        attributes: {
          ...
          ...(this.storageFreeBytes !== undefined && { [K.STORAGE_FREE]: this.storageFreeBytes }),
        },
      });
    };
...
test('Unhandled rejection lands as non_fatal', async ({ page }) => {
  ...
  expect(log.attributes['non_fatal.is_manual']).toBe(false);
