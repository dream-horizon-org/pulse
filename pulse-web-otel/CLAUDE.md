...
- Package: `@dreamhorizonorg/pulse-web` (`package.json` `name`), `0.0.3` (pre-release)
- Build: `tsup` → `dist/`
...
- **Screen navigation:** `screen_load` + `screen_session` + `screen_interactive` OTLP **spans** from [`src/instrumentations/navigation.ts`](src/instrumentations/navigation.ts) when enabled. Web emits a separate **`screen_interactive`** span after `screen_load` on cold/reload when Navigation Timing TTI is available — **`tti`** also stays on `screen_load` for backward compat. SPA and BFCache do **not** emit `screen_interactive`. Android does not emit this span — Web/RN parity only. See **`docs/instrumentations/screen-signals/SPEC.md`**. Router hooks call `Pulse.setScreenName` for `screen.name` on other telemetry.
