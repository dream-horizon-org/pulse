...
CREATE TABLE `athena_job` (
  ...
  `query_execution_id` varchar(255) DEFAULT NULL,
...
CREATE TABLE `notification_templates` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `event_name` varchar(255) NOT NULL,
  `channel_type` enum('SLACK','SLACK_WEBHOOK','EMAIL','TEAMS') DEFAULT NULL,
...
CREATE TABLE `projects` (
  ...
  `slug` varchar(100) DEFAULT NULL COMMENT 'Project slug for URL-friendly identifier',
...
CREATE TABLE `tenants` (
  ...
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `gcp_tenant_id` varchar(32) DEFAULT NULL,
...
CREATE TABLE `usage_limit_notifications` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `project_id` varchar(64) NOT NULL,
  `thresholds_notified` json NOT NULL DEFAULT (_latin1'{}'),
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_month` (`project_id`,(date_format(`created_at`,_latin1'%Y-%m'))),
  KEY `idx_created_at` (`created_at`),
...
CREATE TABLE `users` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `user_id` varchar(255) NOT NULL COMMENT 'Unique user identifier (user-{uuid})',
  `email` varchar(255) NOT NULL COMMENT 'User email from Google OAuth',
  ...
  `firebase_uid` varchar(255) DEFAULT NULL COMMENT 'Firebase user ID for authentication',
...
-- Dump completed on 2026-03-26 16:18:01
