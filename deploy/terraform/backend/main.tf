...
resource "aws_launch_template" "pulse_backend" {
  name_prefix   = "pulse-backend-lt-"
  image_id      = var.ami_id
...
resource "aws_lb_target_group" "pulse_backend" {
  ...
  tags = {
    Name    = "pulse-backend-target-group"
    service = "pulse"
  }
}

resource "aws_lb_listener" "pulse_backend" {
...
resource "aws_autoscaling_group" "pulse_backend" {
  name = "pulse-backend-asg"
  ...
  launch_template {
    id      = aws_launch_template.pulse_backend.id
    version = "$Latest"
  }

  tag {
    key                 = "Name"
    value               = "pulse-backend-instance"
