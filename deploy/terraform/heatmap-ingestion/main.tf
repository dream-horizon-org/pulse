terraform {
  ...
  backend "s3" {
    ...
    key          = "terraform/production/heatmap-screenshot-ingestion/terraform.tfstate"
    region       = "ap-south-1"
    use_lockfile = true
  }
}
...
locals {
  component_name = "pulse-heatmap-screenshot-ingestion"
  tag_base = {
    org_name         = "horizon"
...
# Kafka consumer only — no NLB/ALB.
...
resource "aws_autoscaling_group" "ingestion" {
  name = "pulse-heatmap-screenshot-ingestion-asg"
  ...
  tag {
    key                 = "Name"
    value               = "pulse-heatmap-screenshot-ingestion-asg"
