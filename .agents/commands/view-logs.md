...
1. Ask which service to view logs for if not specified. Available Docker services: pulse-server, pulse-ui, pulse-alerts-cron, otel-collector, mysql, clickhouse, openfga (vector only if uncommented in `deploy/docker-compose.yml` and VECTOR_ENABLED=true)
