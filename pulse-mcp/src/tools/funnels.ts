...
export function registerFunnelTools(server: McpServer): void {
  server.tool(
    "list_funnels",
    "List funnels for a project with optional search, status, and tag filters",
    {
    ...
    async ({ projectId, search, status, tags, page, pageSize }) => {
      const data = await getClient().get<unknown>("/v1/funnels", projectId, {
  ...
  server.tool(
    ...
    async ({ projectId, funnelId }) => {
      const data = await getClient().get<unknown>(`/v1/funnels/${funnelId}`, projectId);
      return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
  );
  ...
  server.tool(
    "get_funnel_tags",
    "List all tags used across funnels and journeys in a project",
    { projectId: z.string().describe("Project ID") },
    async ({ projectId }) => {
      const data = await getClient().get<unknown>("/v1/funnels/tags", projectId);
      return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
  );
  ...
  server.tool(
    "get_funnel_filters",
    "List available filter fields for funnel analysis",
    { projectId: z.string().describe("Project ID") },
    async ({ projectId }) => {
      const data = await getClient().get<unknown>("/v1/funnels/filters", projectId);
      return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
  );
}
