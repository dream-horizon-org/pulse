...
export class MockSessionReplayData {
  ...
  static generateSession(id: number): SessionResponse {
    ...
    const userId = isAnonymous
      ? null
      : this.USERS[Math.floor(Math.random() * (this.USERS.length - 1))];
    const deviceInfo =
      this.DEVICES[Math.floor(Math.random() * this.DEVICES.length)];
    const journey =
      this.JOURNEYS[Math.floor(Math.random() * this.JOURNEYS.length)];
    ...
    // Build issue summary
    const hasFrustration = tags.some(
      (t) => t.type === "rage" || t.type === "dead_click",
    );
    const isSlow = tags.some((t) => t.type === "slow");
    const crashed = tags.some(
      (t) => t.type === "network_fail" && t.count && t.count > 2,
    );
    const failedInteractions = tags.some(
      (t) => t.type === "dead_click" || t.type === "network_fail",
    );
  ...
  static generateSessionDetail(sessionId: string): GetSessionDetailResponse {
    ...
    const events: SessionEventResponse[] = [
      ...
      },
      {
        id: "2",
        timestamp: new Date(
          new Date(session.startTime).getTime() + 5000,
        ).toISOString(),
        type: "click",
      ...
      },
      {
        id: "3",
        timestamp: new Date(
          new Date(session.startTime).getTime() + 15000,
        ).toISOString(),
        type: "error",
        target: "window",
        value: {
  ...
  static filterSessions(
    ...
    return sessions.filter((session) => {
      if (
        ...
        session.environment !== filters.environment
      ) {
        return false;
      }
      ...
      if (
        filters.slowSessions &&
        !session.tags.some((t) => t.type === "slow")
      ) {
        return false;
      }
      ...
      if (filters.searchQuery) {
        ...
        const matchesSessionId = session.sessionId
          .toLowerCase()
          .includes(query);
        const matchesJourney = session.journey.some((path) =>
  ...
  static calculateMetrics(sessions: SessionResponse[]) {
    ...
    const sessionsWithIssues = sessions.filter(
      (s) => s.tags.length > 0 || s.errors > 0,
    ).length;

    const issueRate =
      totalSessions > 0 ? (sessionsWithIssues / totalSessions) * 100 : 0;
    ...
    const errorsAndCrashes = sessions.filter(
      (s) => s.errors > 0 || s.tags.some((t) => t.type === "js_error"),
    ).length;
    ...
    const dropOffs = sessions.filter((s) => s.outcome === "drop_off").length;
    ...
    const criticalInteractions = [
      {
        ...
        // Breakdown
        totalAttempts: sessions.filter((s) =>
          s.journey.some((p) => p.includes("/pay")),
        ).length,
        successfulAttempts: Math.round(
          sessions.filter((s) => s.journey.some((p) => p.includes("/pay")))
            .length * 0.55,
        ),
        failedAttempts: Math.round(
          sessions.filter((s) => s.journey.some((p) => p.includes("/pay")))
            .length * 0.45,
        ),
        uniqueUsers: new Set(
          sessions
            .filter((s) => s.journey.some((p) => p.includes("/pay")))
        ...
        // Session context
        sessionsWithThisInteraction: sessions.filter((s) =>
          s.journey.some((p) => p.includes("/pay")),
        ).length,
      ...
      },
      {
        ...
        // Breakdown
        totalAttempts: sessions.filter((s) =>
          s.journey.some((p) => p.includes("/signup") || p.includes("/login")),
        ).length,
        successfulAttempts: Math.round(
          sessions.filter((s) =>
            s.journey.some(
              (p) => p.includes("/signup") || p.includes("/login"),
            ),
          ).length * 0.78,
        ),
        failedAttempts: Math.round(
      ...
      },
      {
        ...
        // Breakdown
        totalAttempts: sessions.filter((s) =>
          s.journey.some((p) => p.includes("/contest")),
        ).length,
        successfulAttempts: Math.round(
          sessions.filter((s) => s.journey.some((p) => p.includes("/contest")))
            .length * 0.92,
        ),
        failedAttempts: Math.round(
          sessions.filter((s) => s.journey.some((p) => p.includes("/contest")))
        ...
        uniqueUsers: new Set(
          sessions
            .filter((s) => s.journey.some((p) => p.includes("/contest")))
            .map((s) => s.userId || s.sessionId),
        ).size,
    ...
    // Business Impact
    const totalUsersInPeriod = new Set(
      sessions.map((s) => s.userId || s.sessionId),
    ).size;
    const affectedUsersCount = new Set(
      sessions
        .filter((s) => s.tags.length > 0 || s.errors > 0)
        .map((s) => s.userId || s.sessionId),
    ).size;
    ...
    const estimatedImpact = {
      ...
      affectedUsersPercentage:
        totalUsersInPeriod > 0
          ? (affectedUsersCount / totalUsersInPeriod) * 100
          : 0,
    ...
    const paySessionsAtLocation = sessions.filter((s) =>
      s.journey.includes("/pay"),
    ).length;
    const signupSessionsAtLocation = sessions.filter((s) =>
      s.journey.includes("/signup"),
    ).length;
    const contestSessionsAtLocation = sessions.filter((s) =>
      s.journey.includes("/contest"),
    ).length;
    ...
    const topIssueHotspots = [
      {
        ...
        hitRate:
          paySessionsAtLocation > 0 ? (23 / paySessionsAtLocation) * 100 : 0,
        uniqueUsers: 18,
        avgStruggleTime: 45,
      ...
      },
      {
        ...
        hitRate:
          signupSessionsAtLocation > 0
            ? (18 / signupSessionsAtLocation) * 100
            : 0,
        uniqueUsers: 14,
        avgStruggleTime: 32,
      ...
      },
      {
        ...
        hitRate:
          contestSessionsAtLocation > 0
            ? (12 / contestSessionsAtLocation) * 100
            : 0,
        uniqueUsers: 10,
        avgStruggleTime: 28,
    ...
    const topErrorPatterns = [
      ...
      },
      {
        ...
        lastSeen: new Date().toISOString(),
        platformBreakdown: [{ platform: "Web", count: 18 }],
      ...
      },
      {
        ...
        platformBreakdown: [{ platform: "Android", count: 8 }],
    ...
    const sessionsWithIssuesCount = sessions.filter(
      (s) => s.tags.length > 0 || s.errors > 0,
    ).length;
    ...
    const comparison = {
      ...
      sessionsWithIssues: {
        ...
        currentPercent:
          totalSessions > 0
            ? (sessionsWithIssuesCount / totalSessions) * 100
            : 0,
        ...
        trend:
          sessionsWithIssuesCount > previousSessionsWithIssuesCount
            ? ("declining" as const)
            : ("improving" as const),
      },
      topDegradedFlows: [
    ...
    const webSessions = sessions.filter(
      (s) => s.device !== "iOS" && s.device !== "Android",
    );
    ...
    const segmentation = {
      byPlatform: [
        {
          ...
          errorRate:
            iosSessions.length > 0
              ? (iosSessions.filter((s) => s.errors > 0).length /
                  iosSessions.length) *
                100
              : 0,
        ...
        },
        {
          ...
          errorRate:
            androidSessions.length > 0
              ? (androidSessions.filter((s) => s.errors > 0).length /
                  androidSessions.length) *
                100
              : 0,
        ...
        },
        {
          ...
          errorRate:
            webSessions.length > 0
              ? (webSessions.filter((s) => s.errors > 0).length /
                  webSessions.length) *
                100
              : 0,
      ...
      byAppVersion: [
        ...
        },
        {
          version: "2.4.8",
          platform: "iOS" as const,
          sessionCount: Math.round(iosSessions.length * 0.1),
          errorRate: 15.8,
        ...
        },
        {
          version: "2.5.0",
          platform: "Android" as const,
          sessionCount: Math.round(androidSessions.length * 0.3),
          errorRate: 18.9,
          releaseDate: "2024-01-15",
          isLatest: false,
        },
      ...
      byCountry: [
        {
          ...
          sessionCount: Math.round(totalSessions * 0.35),
          errorRate: 11.2,
        },
        {
          countryCode: "IN",
          countryName: "India",
          sessionCount: Math.round(totalSessions * 0.2),
          errorRate: 18.5,
        },
        {
          countryCode: "GB",
          countryName: "United Kingdom",
          sessionCount: Math.round(totalSessions * 0.12),
          errorRate: 9.8,
        },
        {
          countryCode: "CA",
          countryName: "Canada",
          sessionCount: Math.round(totalSessions * 0.08),
          errorRate: 10.5,
        },
        {
          countryCode: "DE",
          countryName: "Germany",
          sessionCount: Math.round(totalSessions * 0.06),
          errorRate: 13.2,
        },
      ],
    };
    ...
    const performance = {
      coreWebVitals: {
        lcp: {
          ...
          sessionsPoor: Math.round(totalSessions * 0.2),
        },
        fid: {
          p50: 85,
          p75: 120,
          ...
          sessionsPoor: Math.round(totalSessions * 0.1),
        },
        cls: {
          p50: 0.08,
          p75: 0.15,
      ...
      topRegions: [
        {
          ...
          errorRate: 18.5,
          avgPerformance: 0.72,
          topIssue: "Payment failures",
        },
        {
          country: "India",
          countryCode: "IN",
          sessionCount: Math.round(totalSessions * 0.25),
          errorRate: 22.8,
          avgPerformance: 0.65,
          topIssue: "Slow load times",
        },
        {
        ...
        },
        {
          country: "Canada",
          countryCode: "CA",
          sessionCount: Math.round(totalSessions * 0.07),
          errorRate: 16.3,
          avgPerformance: 0.75,
        },
        {
          country: "Australia",
          countryCode: "AU",
          sessionCount: Math.round(totalSessions * 0.05),
          errorRate: 19.7,
          avgPerformance: 0.68,
          topIssue: "High latency",
        },
        {
          country: "Brazil",
          countryCode: "BR",
          sessionCount: Math.round(totalSessions * 0.04),
          errorRate: 24.5,
          avgPerformance: 0.62,
          topIssue: "Network timeouts",
        },
        {
          country: "Japan",
          countryCode: "JP",
      ...
      browserBreakdown: [
        {
          browser: "Chrome",
          version: "121",
          sessionCount: Math.round(totalSessions * 0.52),
          errorRate: 17.2,
          compatibilityIssues: 12,
          topIssue: "WebGL crashes",
        },
        {
          browser: "Safari",
          version: "17",
          sessionCount: Math.round(totalSessions * 0.28),
          errorRate: 19.8,
          compatibilityIssues: 24,
          topIssue: "Webkit rendering bugs",
        },
        {
          browser: "Firefox",
          version: "122",
        ...
        {
          browser: "Opera",
          sessionCount: Math.round(totalSessions * 0.02),
          errorRate: 21.2,
      ...
      deviceTypeBreakdown: [
        {
          ...
          sessionCount: Math.round(totalSessions * 0.62),
          errorRate: 21.5,
          avgPerformance: 0.68,
        },
        {
          deviceType: "desktop" as const,
          sessionCount: Math.round(totalSessions * 0.32),
          errorRate: 14.8,
          avgPerformance: 0.78,
        },
        {
      ...
      filters: [
        {
          ...
          description:
            "Sessions where interactions did not complete successfully",
          icon: "alert-circle",
          filterCondition: {
        ...
        },
        {
          ...
          description:
            "Sessions with slow load times or poor interaction quality",
          icon: "clock",
          filterCondition: {
