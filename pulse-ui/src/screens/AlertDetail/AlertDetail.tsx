...
import {
  ...
  isNetworkApiScopeName,
} from "../AlertFormWizard/utils/scopeNameUtils";
...
export function AlertDetail(_props: AlertDetailProps) {
  ...
  const { alertId, projectId } = useParams<{
    alertId: string;
    projectId: string;
  }>();
  ...
  const { data: evaluationHistoryData, isLoading: isHistoryLoading } =
  ...
  const severities =
    severitiesData?.data && Array.isArray(severitiesData.data)
      ? severitiesData.data
      : [];
  severities.forEach((s: { severity_id: number; name: number }) => {
    severityConfig[s.severity_id] = {
  ...
  metricsData?.data?.metrics?.forEach((m) => {
    ...
    onSettled: (data, error) => {
      ...
      if (error || data?.error) {
        showNotification(
          ...
          data?.error?.message || "Failed to snooze",
          <IconSquareRoundedX />,
          theme.colors.red[6],
        );
        return;
  ...
  const resumeAlertMutation = useResumeAlert({
    onSettled: (data, error) => {
      ...
      if (error || data?.error) {
        showNotification(
          COMMON_CONSTANTS.ERROR_NOTIFICATION_TITLE,
          data?.error?.message || "Failed to resume",
          <IconSquareRoundedX />,
          theme.colors.red[6],
        );
        return;
  ...
  const handleEdit = () =>
    navigate(
      `${ROUTES.PROJECT_ALERTS_FORM.basePath.replace(":projectId", projectId || "")}/${alertId}`,
    );
  ...
  const handleSnooze = (durationMs: number) => {
    ...
    snoozeAlertMutation.mutate({
      alertId,
      snoozeAlertRequest: { snoozeFrom: now, snoozeUntil: now + durationMs },
    });
  };
  ...
  const severity = severityConfig[alert?.severity_id || 1] || {
    label: "Unknown",
    color: "#6b7280",
  };
  ...
  const StatusIcon = alert.is_snoozed
    ? IconBellOff
    : isFiring
      ? IconBellRinging
      : IconBell;
  const statusColor = alert.is_snoozed
    ? "#94a3b8"
    : isNoData
      ? "#9ca3af"
      : isFiring
        ? "#ef4444"
        : "#10b981";
  const statusLabel = alert.is_snoozed
    ? "Snoozed"
    : isNoData
      ? "No Data"
      : alert.status;
    ...
    <div className={classes.pageContainer}>
        ...
        <div className={classes.snoozeBanner}>
          <div className={classes.snoozeBannerContent}>
            ...
            <div className={classes.snoozeBannerText}>
              <span className={classes.snoozeBannerTitle}>
                Alert is Snoozed
              </span>
              {alert.snoozed_until && (
                <span className={classes.snoozeBannerUntil}>
          ...
          <div className={classes.heroInfo}>
            <div className={classes.heroBadges}>
              <Badge size="sm" variant="light" color="gray">
                {scopeLabels[alert.scope] || alert.scope}
              </Badge>
              <Badge
                size="sm"
                variant="light"
            ...
            {alert.description && (
              <p className={classes.heroDescription}>{alert.description}</p>
            )}
          </div>
        </div>
        <div className={classes.heroActions}>
          <Tooltip label="Edit Alert">
            <Button variant="light" color="teal" onClick={handleEdit}>
              <IconEdit size={18} />
            </Button>
          </Tooltip>
          {!alert.is_snoozed && (
      ...
      <div className={classes.contentGrid}>
        ...
        <div className={classes.mainContent}>
          ...
          <section className={classes.section}>
            <h2 className={classes.sectionTitle}>
              <IconActivity size={18} /> Alert Conditions
            </h2>
            <p className={classes.sectionSubtitle}>
              Expression:{" "}
              <code className={classes.expressionCode}>
              ...
              {alert.alerts?.map((condition, idx) => (
                <div key={idx} className={classes.conditionCard}>
                  <div className={classes.conditionHeader}>
                    <span className={classes.conditionAlias}>
                      {condition.alias}
                    </span>
                    <span className={classes.conditionMetric}>
                      {metricLabels[condition.metric] || condition.metric}
                    </span>
                    <span className={classes.conditionOperator}>
                      {OPERATOR_SYMBOLS[condition.metric_operator] ||
                      ...
                      ([name, value]) => {
                        ...
                        return (
                          <div key={name} className={classes.thresholdItem}>
                            ...
                            >
                              <span className={classes.thresholdName}>
                                {displayName}
                              </span>
                            </Tooltip>
                            <span className={classes.thresholdValue}>
                              {value}
                            </span>
                          </div>
                        );
          ...
          <section className={classes.section}>
            <h2 className={classes.sectionTitle}>
              <IconClock size={18} /> Evaluation Settings
            </h2>
            <div className={classes.configGrid}>
              <div className={classes.configItem}>
                ...
                <span className={classes.configValue}>
                  {formatDuration(alert.evaluation_period)}
                </span>
              </div>
              <div className={classes.configItem}>
                <span className={classes.configLabel}>Check Interval</span>
                <span className={classes.configValue}>
                  {formatDuration(alert.evaluation_interval)}
                </span>
              </div>
              <div className={classes.configItem}>
            ...
            <div className={classes.metadataGrid}>
              ...
              {alert.updated_by && (
                <div className={classes.metadataItem}>
                  <IconUser size={14} />
                  <span>
                    Updated by <strong>{alert.updated_by}</strong>
                  </span>
                </div>
              )}
              <div className={classes.metadataItem}>
        ...
        <aside className={classes.sidebar}>
          <section className={classes.section}>
            <h2 className={classes.sectionTitle}>
              <IconClock size={18} /> Evaluation History
            </h2>
              ...
              {isHistoryLoading ? (
                ...
                <div className={classes.scopeHistoryList}>
                  {evaluationHistoryData.data.map((scopeHistory, scopeIdx) => {
                    const isExpanded = expandedScopes.has(
                      scopeHistory.scope_id,
                    );
                    const latestStatus = getLatestStatus(
                      scopeHistory.evaluation_history,
                    );
                    ...
                    return (
                      <div
                        key={scopeIdx}
                        className={classes.scopeHistorySection}
                      >
                        {/* Collapsible Header */}
                        <button
                          className={classes.scopeHistoryHeader}
                          onClick={() =>
                            toggleScopeExpanded(scopeHistory.scope_id)
                          }
                        >
                          <div className={classes.scopeHeaderLeft}>
                            ...
                            <Tooltip
                              label={scopeHistory.scope_name}
                              position="top"
                            >
                              <span className={classes.scopeNameText}>
                                {(() => {
                                  // Format network_api scope names for display
                                  const displayName = isNetworkApiScopeName(
                          ...
                          <span className={classes.scopeHistoryCount}>
                            {scopeHistory.evaluation_history.length} eval
                            {scopeHistory.evaluation_history.length !== 1
                              ? "s"
                              : ""}
                          </span>
                        </button>
                                    ...
                                    <div className={classes.timelineContent}>
                                      <div className={classes.timelineHeader}>
                                        ...
                                        <span className={classes.timelineTime}>
                                          {formatTime(
                                            new Date(item.evaluated_at),
                                          )}
                                        </span>
                                      </div>

                                      {/* Metric Readings */}
                                      {Object.keys(evaluationResult).length >
                                        0 && (
                                        <div className={classes.scopeReadings}>
                                          <div
                                            className={
                                              classes.scopeReadingsHeader
                                            }
                                          >
                                            <span>Metric</span>
                                            <span>Value</span>
                                          ...
                                          {Object.entries(evaluationResult).map(
                                            ([metricKey, value], mrIdx) => (
                                              <div
                                                key={mrIdx}
                                                className={
                                                  classes.scopeReadingRow
                                                }
                                              >
                                                <span
                                                ...
                                                <span
                                                  className={`${classes.scopeValue} ${isItemFiring ? classes.scopeValueFiring : classes.scopeValueNormal}`}
                                                >
                                                  {value}
                                                </span>
                                              </div>
