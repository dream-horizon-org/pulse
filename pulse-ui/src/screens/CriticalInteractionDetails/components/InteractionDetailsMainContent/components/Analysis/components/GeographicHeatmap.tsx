...
const GeographicHeatmap: React.FC<GeographicHeatmapProps> = ({
  ...
  const trustedRows = visibleData.filter((d) => (d.total ?? Infinity) >= 5);
  const refValues = (trustedRows.length > 0 ? trustedRows : visibleData).map(
    (d) => d.value,
  );
  ...
  const renderLocationRow = (location: GeographicLocation) => {
    ...
    return (
      <Tooltip
        ...
        label={
          totalLabel
            ? `${displayValue} ${metricLabel} • ${totalLabel}`
    ...
    >
      ...
      <Box h={334} px="md" style={{ overflowY: "auto" }}>
        <Box style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {visibleData.map((location) => renderLocationRow(location))}
        </Box>
      </Box>
