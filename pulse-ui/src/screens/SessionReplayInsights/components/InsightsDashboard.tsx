    ...
    <Stack gap="lg" className={classes.dashboard}>
      ...
      {estimatedImpact &&
        (estimatedImpact.totalRevenueAtRisk > 0 ||
          estimatedImpact.affectedUsers > 0) && (
                ...
                {estimatedImpact.conversionImpact > 0 && (
                  <MetricCard
                    ...
                    onClick={() =>
                      onDrillDown?.(
                        "conversion_loss",
                        estimatedImpact.conversionImpact,
                        "Conversion Loss Sessions",
                      )
                    }
                  />
                )}
                ...
                {estimatedImpact.supportTicketCorrelation && (
                  <MetricCard
                    ...
                    badge={
                      ...
                      >
                        {estimatedImpact.supportTicketCorrelation.confidence}{" "}
                        confidence
                      </Badge>
                    }
                    onClick={() =>
          ...
          <ComparisonMetricCard
            ...
            onClick={() =>
              onDrillDown?.(
                "sessions_with_issues",
                "all",
                "Sessions with Failed Interactions",
              )
            }
            ...
            <SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }} spacing="md">
              {criticalInteractions.map((interaction) => (
                <StatusMetricCard
                  ...
                  status={
                    interaction.healthStatus.toLowerCase() as
                      | "excellent"
                      | "good"
                      | "fair"
                      | "poor"
                  ...
                  subMetrics={[
                    {
                      label: "Error Rate",
                      value: `${interaction.errorRate.toFixed(1)}%`,
                    },
                    { label: "P50", value: `${interaction.p50Latency}ms` },
                    {
                      label: "Sessions",
                      value: interaction.sessionsWithThisInteraction,
                    },
                  ]}
                  ...
                  onClick={() =>
                    onDrillDown?.(
                      ...
                      `${interaction.displayName} Sessions`,
                    )
                  }
              ...
              <Stack gap="md">
                <div>
                  ...
                  <Text size="xs" c="dimmed" mt={4}>
                    Where users struggle with interactions (top 3)
                  </Text>
                </div>

                {topIssueHotspots && topIssueHotspots.length > 0 ? (
                  <Timeline
                    active={Math.min(topIssueHotspots.length, 3)}
                    bulletSize={20}
                    lineWidth={2}
                  >
                    {topIssueHotspots.slice(0, 3).map((hotspot, index) => (
                      ...
                      >
                        <Stack gap="xs" mt="xs">
                          ...
                          <Group gap="md">
                            <Text size="xs" c="dimmed">
                              {hotspot.affectedSessions} sessions
                            </Text>
                            <Text size="xs" c="dimmed">
                              {hotspot.hitRate.toFixed(1)}% hit rate
                            </Text>
                            <Text size="xs" c="dimmed">
                              {hotspot.medianStruggleTime}s struggle
                            </Text>
                          </Group>
                          <DrillDownButton
            ...
            >
              <Stack gap="md">
                <div>
                  ...
                  <Text size="xs" c="dimmed" mt={4}>
                    Failed interactions by error type (top 3)
                  </Text>
                </div>
                      ...
                      <Card key={index} padding="md" radius="md" withBorder>
                        <Stack gap="xs">
                          <Group gap="xs">
                            <Badge
                              size="sm"
                              color={getErrorTypeColor(pattern.errorType)}
                              variant="filled"
                            >
                              {pattern.errorType}
                            </Badge>
                          ...
                          <Group gap="md">
                            ...
                            <Text size="xs" c="dimmed">
                              <strong>{pattern.affectedSessions}</strong>{" "}
                              sessions
                            </Text>
                            <Text size="xs" c="dimmed">
                          ...
                          {pattern.platformBreakdown &&
                            pattern.platformBreakdown.length > 0 && (
                              <Group gap="xs">
                                {pattern.platformBreakdown.map(
                                  (platformObj) => (
                                    <Badge
                                      key={platformObj.platform}
                                      size="xs"
                                      variant="dot"
                                      color="gray"
                                    >
                          ...
                          <Group gap="xs" grow>
                            <DualDrillDown
                              ...
                              onPrimaryClick={() =>
                                onViewSession?.(pattern.sampleSessionId)
                              }
                              onSecondaryClick={() =>
                                onDrillDown?.(
                                  "error_pattern",
                                  pattern.errorSignature,
                                  `Error: ${pattern.displayName}`,
                                )
                              }
                              color={getSeverityColor(pattern.severity)}
                            />
      ...
      {/* ========================================
          ...
          ======================================== */}
      {timePatterns &&
        (timePatterns.peakErrorHours?.length > 0 ||
          timePatterns.errorTrend?.length > 0) && (
          <Paper p="lg" radius="md" id="time-patterns">
            <Stack gap="md">
              ...
              {timePatterns.peakErrorHours &&
                timePatterns.peakErrorHours.length > 0 && (
                  <Stack gap="sm">
                    <Text size="sm" fw={600}>
                      Peak Error Hours (Top 3)
                    </Text>
