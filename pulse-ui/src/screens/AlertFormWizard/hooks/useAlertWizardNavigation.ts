...
import {
  StepInfo,
  NavigationResult,
  UseAlertWizardNavigationReturn,
} from "./useAlertWizardNavigation.interface";
...
export const useAlertWizardNavigation = (): UseAlertWizardNavigationReturn => {
  const {
    ...
    stepValidation,
    isEditMode,
    goToStep: ctxGoToStep,
    goToNextStep: ctxGoToNextStep,
    goToPreviousStep: ctxGoToPrevious,
    validateStep,
  } = useAlertFormContext();
  const { currentStep, visitedSteps, effectiveSteps: stepIds } = navigation;

  const effectiveSteps = useMemo(
    (): StepInfo[] => stepIds.map((id) => ({ id, ...STEP_CONFIGS[id] })),
    [stepIds],
  );
  const currentStepIndex = stepIds.indexOf(currentStep);
  const currentStepInfo = useMemo(
    () => ({ id: currentStep, ...STEP_CONFIGS[currentStep] }),
    [currentStep],
  );
  const progressPercentage = Math.round(
    ((currentStepIndex + 1) / stepIds.length) * 100,
  );
  const isCurrentStepValid = stepValidation[currentStep]?.isValid ?? false;

  const isStepVisited = useCallback(
    (step: WizardStep) => visitedSteps.has(step),
    [visitedSteps],
  );
  ...
  const isStepAccessible = useCallback(
    (step: WizardStep): boolean => {
      ...
      for (let i = 0; i < idx; i++)
        if (!visitedSteps.has(stepIds[i])) return false;
      return true;
    },
    [isEditMode, stepIds, visitedSteps],
  );
  ...
  const goToStep = useCallback(
    (step: WizardStep, skipValidation = false): NavigationResult => {
  ...
  const goToNextStep = useCallback(
    (skipValidation = false): NavigationResult => {
      if (!navigation.canGoNext)
        return { success: false, message: "Already at last step." };
      if (!skipValidation) {
        const result = validateCurrentStep();
        if (!result.isValid)
          return { success: false, validationErrors: result.errors };
  ...
  const goToFirstInvalidStep = useCallback((): NavigationResult => {
    for (const id of stepIds) {
      if (!stepValidation[id]?.isValid) {
        ctxGoToStep(id);
        return {
          success: true,
          targetStep: id,
          validationErrors: stepValidation[id]?.errors,
        };
      }
    }
