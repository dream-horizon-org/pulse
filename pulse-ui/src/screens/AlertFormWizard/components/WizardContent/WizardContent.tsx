...
import {
  ...
  transformAlertDetailsToFormData,
} from "../../AlertFormWizard.utils";
...
export const WizardContent: React.FC<WizardContentProps> = ({
  ...
  const { formData, isEditMode, isLoading, setFormData } =
    useAlertFormContext();
  ...
  }, [isInteractionDetailsFlow, onBackButtonClick, navigate, projectId]);
  ...
  const {
    ...
    error,
  } = useGetAlertDetails({ queryParams: { alert_id: alertId || null } });
  ...
  useEffect(() => {
    if (alertDetailsResponse?.data && isEditMode && !hasPopulatedForm.current) {
      ...
      const formDataFromDetails = transformAlertDetailsToFormData(
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!email) {
      showError(COMMON_CONSTANTS.USER_EMAIL_NOT_FOUND);
      return;
    }
    setShowLoader(true);
    const payload = transformFormDataToPayload(formData, email);
    isEditMode
      ? updateMutation.mutateAsync(payload as never)
      : createMutation.mutateAsync(payload as never);
  ...
  return (
    <Box className={classes.wizardContainer}>
      ...
      <Box className={classes.wizardContent}>
        <Box className={classes.stepperSidebar}>
          <WizardStepper />
        </Box>
        <Box className={classes.stepContentArea}>
          <StepRenderer />
        </Box>
      </Box>
      <StepNavigation
        ...
        submitText={isEditMode ? "Update Alert" : "Create Alert"}
        confirmTitle={
          isEditMode ? MODAL_TEXTS.UPDATE_TITLE : MODAL_TEXTS.CREATE_TITLE
        }
        confirmMessage={
          isEditMode ? MODAL_TEXTS.UPDATE_MESSAGE : MODAL_TEXTS.CREATE_MESSAGE
        }
      />
      <DeleteModal
