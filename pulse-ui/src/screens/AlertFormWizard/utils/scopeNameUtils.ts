 ...
 * e.g., "get_https://www.example.com/graphql"
 */
 ...
 */
export function parseNetworkApiScopeName(
  scopeName: string,
): ParsedNetworkScopeName | null {
 ...
 * Converts "get_https://www.example.com/graphql" to "GET https://www.example.com/graphql"
 */
 ...
 * Converts "get_https://www.example.com/v1/contests/live" to "GET .../contests/live"
 */
export function formatNetworkApiScopeNameCompact(
  scopeName: string,
  maxUrlLength: number = 30,
): string {
  const parsed = parseNetworkApiScopeName(scopeName);
  if (!parsed) {
  ...
  try {
    ...
    const displayPath =
      path.length > maxUrlLength ? `...${path.slice(-maxUrlLength)}` : path;
    return `${method.toUpperCase()} ${displayPath}`;
  } catch {
    // If URL parsing fails, just truncate
    const displayUrl =
      url.length > maxUrlLength ? `...${url.slice(-maxUrlLength)}` : url;
