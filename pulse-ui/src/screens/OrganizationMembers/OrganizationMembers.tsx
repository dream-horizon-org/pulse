...
import { TenantMember, BulkInviteResult } from "../../types/members";
import {
  useTenantMembers,
...
export function OrganizationMembers() {
  ...
  const handleInviteMember = () => {
    if (inviteEmails.length === 0 || !tenantId) return;
    ...
    inviteMutation.mutate(
      ...
      {
        onSuccess: (response: ApiResponse<TenantMember | BulkInviteResult>) => {
                        ...
                        canUpdateTenantRoles ? (
                          <Group gap="xs">
                            <Select
                              ...
                              value={newRole}
                              onChange={(val) =>
                                setNewRole((val as TenantRole) || member.role)
                              }
                              data={[
                                {
                                  value: TENANT_ROLES.ADMIN,
                                  label: TENANT_ROLE_LABELS[TENANT_ROLES.ADMIN],
                                },
                                {
                                  value: TENANT_ROLES.MEMBER,
                                  label:
                                    TENANT_ROLE_LABELS[TENANT_ROLES.MEMBER],
                                },
                              ]}
                              style={{ width: 120 }}
      ...
      >
        <Stack gap="md">
          ...
          <Select
            ...
            value={inviteRole}
            onChange={(value) =>
              setInviteRole((value as TenantRole) || TENANT_ROLES.MEMBER)
            }
            data={[
              {
                value: TENANT_ROLES.ADMIN,
              ...
              {
                value: TENANT_ROLES.MEMBER,
                label: TENANT_ROLE_LABELS[TENANT_ROLES.MEMBER],
              },
            ]}
          />
          <Button
