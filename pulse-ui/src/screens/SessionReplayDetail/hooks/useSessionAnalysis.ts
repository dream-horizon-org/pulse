...
function detectSessionType(data: SessionDetailData): SessionType {
  ...
  const hasFailedInteractions = data.criticalInteractions.some(
    (i) => i.status === "failed",
  );
  ...
  const hasSlowInteractions = data.performance.interactionMetrics.some(
    (m) => m.apdexScore < 0.5,
  );
...
function detectTimeoutErrors(data: SessionDetailData): DetectedIssue[] {
  ...
  data.exceptions.rows.forEach((row, idx) => {
    ...
    if (
      ...
      exceptionMessage.includes("timeout")
    ) {
      const timestamp =
        new Date(row[0] as string).getTime() -
...
function detectSlowInteractions(data: SessionDetailData): DetectedIssue[] {
  ...
  data.performance.interactionMetrics.forEach((metric) => {
    if (metric.apdexScore < 0.5) {
      const interaction = data.criticalInteractions.find(
        (i) => i.interactionId === metric.interactionId,
      );
      ...
      issues.push({
        ...
        suggestedAction:
          "Investigate API performance or optimize client-side processing",
      });
    }
  });
...
function detectAPIFailures(data: SessionDetailData): DetectedIssue[] {
  ...
  data.networkRequests.forEach((req, idx) => {
    if (req.status >= 400 && req.status !== 504) {
      // Not timeout (handled above)
  ...
  data.consoleLogs.forEach((log, idx) => {
    if (log.level === "error") {
      issues.push({
        ...
        userFacingImpact:
          "App encountered an error. User may have seen error message or crash.",
        technicalCause:
...
function generateSupportSummary(
  ...
  return {
    ...
    description:
      mainIssue.userFacingImpact +
      (otherCount > 0
        ? ` (Plus ${otherCount} other issue${otherCount > 1 ? "s" : ""})`
        : ""),
    severity: "error",
    actionable: true,
    suggestedActions: [
...
function generateProductSummary(
  ...
  const failedInteractions = data.criticalInteractions.filter(
    (i) => i.status === "failed",
  );
  ...
  if (sessionType === "conversion_success") {
    return {
      ...
      description:
        "User successfully completed their goal. This is a successful session.",
      impact: "positive",
      metrics: {
        actualDuration: formatDuration(data.duration),
  ...
  if (sessionType === "error_encountered") {
    return {
      ...
      description:
        "Technical error prevented user from completing their action",
      impact: "high",
      metrics: {
        similarDropoffsToday: data.businessContext?.similarErrorsToday || 0,
      },
  ...
  if (sessionType === "performance_issue") {
    return {
      ...
      description:
        "User experienced slow interactions but may have completed their goal",
      impact: "medium",
      metrics: {
        actualDuration: formatDuration(data.duration),
      },
  ...
  return {
    ...
    description:
      "User browsing without clear conversion intent. No critical issues detected.",
    impact: "neutral",
  };
}

function generateTechSummary(
  data: SessionDetailData,
  issues: DetectedIssue[],
): TechSummary {
  const errors = data.exceptions.rows;
  const slowInteractions = data.performance.interactionMetrics.filter(
    (m) => m.apdexScore < 0.5,
  );
