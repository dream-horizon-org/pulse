...
export function SamplingRulesConfig({
  config,
  onChange,
  disabled = false,
}: SamplingConfigProps) {
  // Fetch dynamic options from backend
  const { data: rulesAndFeatures, isLoading: isLoadingRules } =
    useGetSdkRulesAndFeatures();
  const { data: scopesAndSdks, isLoading: isLoadingSdks } =
    useGetSdkScopesAndSdks();
  ...
  const getPlaceholder = (rule: SamplingRuleName | "") => {
    switch (rule) {
      case "app_version":
        return "e.g., 2.0.0 or 2\\..*";
      case "os_version":
        return "e.g., 14 or 15\\..*";
      case "country":
        return "e.g., US, IN, GB (ISO country code)";
      case "state":
        return "e.g., CA, NY (ISO region code)";
      case "device":
        return "e.g., Samsung.*, Pixel.*, iPhone.*";
      case "network":
        return "e.g., wifi, cellular, 4g";
      case "platform":
        return "e.g., android, ios";
      default:
        return "Enter value or regex pattern";
    }
  };
    ...
    <>
      <Box className={classes.card}>
          ...
          <Box className={classes.cardHeaderLeft}>
            ...
            <Box>
              <Text className={classes.cardTitle}>
                {UI_CONSTANTS.SECTIONS.SAMPLING.TITLE}
              </Text>
              <Text className={classes.cardDescription}>
                {UI_CONSTANTS.SECTIONS.SAMPLING.DESCRIPTION}
              </Text>
            </Box>
          </Box>
        </Box>
        ...
        <Box className={classes.cardContent}>
          ...
          >
            <Text size="xs">
              Session sampling randomly selects a percentage of user sessions to
              track. For example, at <strong>50%</strong>, half of your users
              will have their events recorded, while the other half will not
              generate any telemetry data.
            </Text>
            <Text size="xs" mt="xs" c="dimmed">
              💡 <strong>Tip:</strong> Lower sampling reduces data volume and
              costs. Start at 50% and adjust based on your debugging needs. Use
              rules to sample differently based on device parameters.
            </Text>
          </Alert>
          ...
          <Box mb="xl">
              ...
              <Box>
                <Group gap="xs">
                  ...
                  >
                    <IconInfoCircle
                      size={14}
                      style={{ color: "#868e96", cursor: "help" }}
                    />
                  </Tooltip>
                </Group>
            ...
            <Slider
              ...
              styles={{
                ...
                bar: {
                  background:
                    "linear-gradient(90deg, #0ec9c2 0%, #0ba09a 100%)",
                },
                thumb: {
                  borderColor: "#0ba09a",
            ...
            <Text size="xs" c="dimmed" mt="xl" ta="center">
              At {Math.round(config.default.sessionSampleRate * 100)}%,
              approximately{" "}
              {Math.round(config.default.sessionSampleRate * 1000)} out of 1,000
          ...
          <Divider
            my="lg"
            label="Conditional Sampling Rules"
          ...
          <Group justify="space-between" mb="md">
            <Box>
              <Group gap="xs">
                <Text fw={600}>
                  {UI_CONSTANTS.SECTIONS.SAMPLING_RULES.TITLE}
                </Text>
                <Tooltip
              ...
              <Text size="xs" c="dimmed">
                {UI_CONSTANTS.SECTIONS.SAMPLING_RULES.DESCRIPTION}
              </Text>
            </Box>
            {!disabled && (
              <Button
          ...
          {config.rules.length === 0 ? (
            <Box className={classes.emptyState}>
              ...
              <Text size="xs" c="dimmed">
                Default rate will be applied to all sessions
              </Text>
              <Paper
                p="sm"
                mt="md"
              ...
              >
                <Group gap="xs" wrap="nowrap">
                  ...
                  <Text size="xs" c="yellow.8">
                    <strong>Example:</strong> Sample 100% of users on
                    app_version &gt;= 2.0.0 to catch bugs in new releases, while
                    keeping 10% for older versions to save costs.
                  </Text>
                </Group>
              </Paper>
            </Box>
          ...
          <Stack gap="md">
            ...
            <Group justify="flex-end" mt="md">
              ...
              <Button
                ...
                disabled={
                  !ruleName || !ruleValue.trim() || ruleSdks.length === 0
                }
              >
