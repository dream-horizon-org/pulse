...
export interface HeatmapAggregateSnapshot {
  ...
  errorClickBins: number;
  latencyHotspotBins: number;
}
...
export function buildHeatmapAggregateSnapshot(
  ...
  const err = payload.layers.observability_map?.error_clicks ?? [];
  const lat = payload.layers.observability_map?.latency_hotspots ?? [];
  ...
  return {
    ...
    totalEventsReported:
      payload.metadata.total_events != null
        ...
        : null,
    errorClickBins: err.length,
    latencyHotspotBins: lat.length,
  };
}
