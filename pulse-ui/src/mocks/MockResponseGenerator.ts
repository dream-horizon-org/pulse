...
export class MockResponseGenerator {
  ...
  /**
   * Mock response for GET /v1/interactions/:name/root-cause (Root Cause Analysis).
   */
  private getMockRootCauseResponse(
  ...
  private handleJobEndpoints(
    ...
    if (
      ...
      if (
        ...
        pathParts[2] &&
        pathParts[3] === "root-cause"
      ) {
        const interactionName = decodeURIComponent(pathParts[2]);
        const mockRootCause = this.getMockRootCauseResponse(interactionName);
        return { data: mockRootCause, status: 200 };
      }
