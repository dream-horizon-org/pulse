...
import {
  ...
  GetAlertMetricsParams,
} from "./useGetAlertMetrics.interface";
...
export const useGetAlertMetrics = ({ scope }: GetAlertMetricsParams) => {
  ...
  const enabled = useProjectQueryEnabled(!!scope);

  return useQuery({
    queryKey: [getAlertMetrics.key, scope],
    queryFn: async () => {
      if (!scope) {
        return { data: { scope: "", metrics: [] } } as {
          data: GetAlertMetricsResponse;
        };
      }
