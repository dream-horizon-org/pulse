...
- Click Tracking (View + Compose)
    - Both click instrumentations are bundled via `pulse-android-sdk` in this demo app; no extra click dependency is required.
    - Emitted events include click coordinates, context labels (if enabled), dead clicks, and rage-click metadata.
    - On first launch, click is suppressed until SDK config is fetched/applied; backend `click` feature controls runtime enablement afterward.
