...
// Forward declarations.
    ...
    public:
        ...
        void set_cur_offset(uint64_t cur_offset) {
            cur_offset_ = cur_offset;
        }

        void set_pc_offset(int64_t offset) {
            pc_offset_ = offset;
        }

        void clear_pc_offset() {
            pc_offset_ = INT64_MAX;
        }
        ...
        void clear_data_offset() {
            data_offset_ = static_cast<uint64_t>(-1);
        }

        void set_func_offset(uint64_t offset) {
            func_offset_ = offset;
        }

        void clear_func_offset() {
            func_offset_ = static_cast<uint64_t>(-1);
        }

        void set_text_offset(uint64_t offset) {
            text_offset_ = offset;
        }

        void clear_text_offset() {
            text_offset_ = static_cast<uint64_t>(-1);
        }
