...
Memory* MapInfo::CreateMemory(const std::shared_ptr<Memory>& process_memory) {
  ...
  // break something.
  //
  // Pulse: Some Android builds map the r-x segment with an empty "pathname"
  // column while the preceding r-- map carries the real .so path. The stock
  // check `name().empty()` short-circuited here and skipped the merge, leaving
  // CreateMemory(nullptr) → invalid Elf (ERROR_INVALID_ELF).
  if (offset() == 0 || prev_real_map() == nullptr || prev_real_map()->offset() >= offset()) {
    set_memory_backed_elf(false);
    return nullptr;
  }
  const bool prev_named = !prev_real_map()->name().empty();
  const bool cur_named = !name().empty();
  if (!prev_named && !cur_named) {
    set_memory_backed_elf(false);
    return nullptr;
  }
  if (cur_named && prev_named && prev_real_map()->name() != name()) {
    set_memory_backed_elf(false);
    return nullptr;
  }
  if (cur_named && !prev_named) {
    set_memory_backed_elf(false);
    return nullptr;
  }
