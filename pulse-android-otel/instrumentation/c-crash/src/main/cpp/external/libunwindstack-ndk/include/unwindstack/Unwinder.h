...
// Forward declarations.
    ...
    public:
        ...
        std::shared_ptr<Memory> &GetProcessMemory() {
        ...
        void SetEmbeddedSoname(bool embedded_soname) {
            embedded_soname_ = embedded_soname;
        }

        void SetDisplayBuildID(bool display_build_id) {
            display_build_id_ = display_build_id;
        }

        void SetDexFiles(DexFiles *dex_files);

        bool elf_from_memory_not_file() {
            return elf_from_memory_not_file_;
        }
        ...
        const char *LastErrorCodeString() {
            return GetErrorCodeString(last_error_.code);
        }
