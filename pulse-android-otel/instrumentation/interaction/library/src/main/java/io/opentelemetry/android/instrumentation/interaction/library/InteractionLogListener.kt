...
class InteractionLogListener(private val interactionManager: InteractionManager) :
    LogRecordProcessor {
    override fun onEmit(
        context: Context,
        ...
        if (logRecord.eventName in listOfEventToAddInInteraction) {
            ...
            interactionManager.addMarkerEvent(
                eventName = logRecord.attributes[PulseAttributes.PULSE_TYPE]
    ...
    companion object {
        private val listOfEventToAddInInteraction = listOf(
            "device.crash",
            "device.anr",
            "app.jank",
            "network.change",
