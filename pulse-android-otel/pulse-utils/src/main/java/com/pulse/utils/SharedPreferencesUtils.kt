 ...
 * is canceled.
 */
public inline fun <reified T> SharedPreferences.toFlow(key: String): Flow<T?> =
    callbackFlow {
...
internal inline fun <reified T> SharedPreferences.readValue(
    ...
    val result =
        when (T::class) {
            ...
            Boolean::class -> {
                if (contains(key)) getBoolean(key, defaultValue as? Boolean == true) else defaultValue
            }
            Set::class -> {
                ...
                error("Unsupported SharedPreferences type: ${T::class}")
            }
        }
